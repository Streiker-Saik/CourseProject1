...
from src.reports import calculate_date_range, spending_by_category, spending_by_weekday


@pytest.mark.parametrize(
    "date, number_of_days, expected",
    [
        ("2025-03-11", 90, (datetime.datetime(2025, 3, 11),
                            datetime.datetime(2024, 12, 11))),
        ("2025-03-11", 10, (datetime.datetime(2025, 3, 11),
                            datetime.datetime(2025, 3, 1))),
        ("2025-03-11", 30, (datetime.datetime(2025, 3, 11),
                            datetime.datetime(2025, 2, 9))),
    ],
)
...
def test_spending_by_category(
        mock_get: MagicMock, mock_date: Tuple[datetime.datetime, datetime.datetime],
        date: Optional[str], category: str, expected: str
) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            (datetime.datetime(2018, 6, 15), datetime.datetime(2018, 3, 17)),
            "2018-06-15",
            '{"Monday": 0, "Tuesday": 0, "Wednesday": 0, "Thursday": 100.93, "Friday": 150.5, "Saturday": 0, "Sunday": 0}',
        ),
        (
...
def test_spending_by_weekday(
        mock_get: MagicMock, mock_date: Tuple[datetime.datetime, datetime.datetime], date: Optional[str], expected: str
) -> None:
    """Тестирование проверяет фильтрацию по категории с датой и при не указании даты"""
    mock_get.return_value = mock_date
